...
class _CircleShapeImageState extends State<CircleShapeImage> {
  ...
  Widget build(BuildContext context) {
    return widget.isSvgImage ? Container(
      ...
      child: CustomSvgPicture(image:widget.image, color: widget.imageColor??MyColor.getPrimaryColor(), height: widget.size*2, width: widget.size *2),
    ) : Container(
      height: widget.size, width: widget.size,
      padding: const EdgeInsets.all(Dimensions.space10),
      alignment: Alignment.center,
      ...
      child: CustomSvgPicture(image:  widget.image, color: widget.imageColor, height: widget.size*2, width: widget.size *2),
    );
  }
}
